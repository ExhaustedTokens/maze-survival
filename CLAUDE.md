...
- **Il codice vive nella "auto-sync directory"** del mondo
  ...
  Cancellare un file dalla cartella non toglie lo script dal mondo: si cancella dall'editor. **Il repo è la fonte di
  verità** (`docs/decisioni.md`): si lavora fuori da quella cartella, si mergia con PR, poi i file `.ts` si copiano
  nella cartella del mondo giusto (staging = mondo clone, produzione = mondo principale). Script di copia, giro
  completo e mondo di staging privato: #22. Il deploy non si automatizza dalla CI.
...
- **Blender** per gli asset: FBX + PNG, niente animazioni importate, niente normal map; convenzioni e budget in
  `docs/pipeline-3d.md` (issue #7, PR #20). L'editor non chiede il tipo di mondo: i mondi nuovi sembrano già
  "custom model world", da confermare importando il primo FBX (#7).
